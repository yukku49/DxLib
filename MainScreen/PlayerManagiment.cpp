...
{
	// Set initial move position (JP: shoki ido ichi)
	Player_MovePointX = Player_StanderdpointX;
	Player_MovePointY = Player_StanderdpointY;

	// Load player images into handles (JP: player image load)
...
void Player_Managiment::Update(const BackScreen& stage, Bllent_Managiment& bllent)
{
	...
	// Move only on new key press (one-tile step) (JP: atarashii input de 1 masu)
	if ((nowUp == 1 && m_oldUp == 0))
	{
		...
		// Save current state for next frame (JP: jikai frame hozon)
		m_oldUp = nowUp;
		m_oldDown = nowDown;
		m_oldLeft = nowLeft;
		m_oldRight = nowRight;
